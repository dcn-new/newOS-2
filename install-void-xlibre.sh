...
base-system grub-x86_64-efi vim dbus NetworkManager acpid xtools

# 5. Generate FSTAB
xgenfstab -U /mnt > /mnt/etc/fstab

# 6. THE HAND-OFF (Copy script into the new root and execute)
cp "$0" /mnt/tmp/install-void-xlibre.sh
chmod +x /mnt/tmp/install-void-xlibre.sh

echo "Entering chroot to execute Stage 2..."
xchroot /mnt /tmp/install-void-xlibre.sh --stage2
